        ...
        @foreach($diario_pensamientos as $diario)
            ...
            @else
                <tr class="table-success">
                    ...
                    <td>{{$diario->texto}}</td>
                    <td>
                        {{ \Carbon\Carbon::parse($diario->created_at)->format("d-m-Y H:i:s") }}
                        <button class="btn btn-sm btn-warning add_comentario" data-id="{{$diario->id}}"
                ...
                <tr class="d-none comentario_pensamiento" id="form_comentario_{{$diario->id}}">
                    ...
                    <td colspan="2">
                        <form method="POST"
                            ...
                            <div class="mb-3">
                                <button id="submit_comentario_{{$diario->id}}" class="btn btn-primary btn-sm d-none"
                                        type="submit">Guardar comentario
                                </button>
                            </div>
                        </form>
                    </td>
